...
<script>
    $(document).ready(function() {
        $('#show_data').on('click', '.item_edit', function() {
            var namaKategori = $(this).data('kategori_nama');
            var kategoriid = $(this).data('kategori_id');
            // var price = $(this).data('price');idJenisIzin

            $('#Modal_Edit').modal('show');
            $('[name="kategori_nama_edit"]').val(namaKategori);
            $('[name="kategori_id_edit"]').val(kategoriid);
        ...
        $('#btn_update').on('click', function() {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('master/ajax_editkat') ?>",
                dataType: "JSON",
                data: {
                ...
                success: function(data) {
                    $('[name="kategori_id_edit"]').val("");
                    $('[name="kategori_nama_edit"]').val("");
                    reload_table();
    ...
    if ($("#ajax_form").length > 0) {
        $("#ajax_form").validate({
            ...
            messages: {
                ...
                nama: {
                    required: "Wajib mengisi nama kategori",
                },
            },
            submitHandler: function(form) {
                $('#send_form').html('Menyimpan...');
                $.ajax({
                    url: "<?php echo base_url('master/t_kat') ?>",
                    type: "POST",
...
<script type="text/javascript">
    $(document).ready(function() {
        table = $('#tabeldata').DataTable({
            ...
            // "paging": false,
            "ajax": {
                "url": "<?php echo site_url('master/ajax_kategori'); ?>",
                "type": "POST"
            },
            "columnDefs": [{
                "targets": [0],
    ...
    function delete_data(id) {
        swal.fire({
            ...
            if (result.value) {
                ...
                $.ajax({
                    url: "<?php echo site_url('master/delete_kat') ?>/" + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data) {
                        Swal.fire({
                            ...
                            width: 200,
                            height: 200,
                            // background: 'linear-gradient(90deg, rgba(69,195,0,0.8575805322128851) 0%, rgba(80,204,5,0.4822303921568627) 100%)',
                            timer: 11500
                        });
